...
API_TOKEN = "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"
...
def main():
    ...
    with st.sidebar:
        ...
        - Clear prompt or history as needed.
        """)
        st.markdown("---")
        st.header("🧙‍♂️ Select Genre")
    ...
    with col2:
        clear_prompt_clicked = st.button("🧹 Clear Prompt")
    with col3:
        clear_history_clicked = st.button("🗑️ Clear History")

    if clear_prompt_clicked:
    ...
    if generate_clicked:
        if prompt.strip() == "":
            st.warning("⚠️ Please enter a valid prompt before generating.")
        else:
                    ...
                    with col_copy:
                        if st.button("📋 Copy to Clipboard"):
                            try:
                                ...
                                st.info("✅ Story copied to clipboard!")
                            except Exception:
                                st.warning("⚠️ Copy to clipboard not supported in this environment.")
    ...
    if st.session_state.get("story_history"):
        st.markdown("---")
        st.subheader("🗂️ Generated Story History")
